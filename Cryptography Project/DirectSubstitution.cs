...
namespace Cryptography_Exercises
{
    static class DirectSubstitution
    {
        ...
        public static Dictionary<char, int> SubstitutionCodes { get; private set; }
        ...
        {
            SubstitutionCodes = new Dictionary<char, int>(M.Length);
        ...
        public static string Decrypt(string cryptogram, bool areSpacesEncrypted = true)
        {
            ...
            if (areSpacesEncrypted)
            {
                ...
                for (int i = 0; i < input.Length; i++)
                {
                    ...
                    if (!isNum)
                        throw new ArgumentException("Криптограмата съдържа символи, които не са числа.");

                    result[i] = SubstitutionCodes.SingleOrDefault(x => x.Value == num).Key;

                    if (result[i] == 0)
                        throw new ArgumentException("Тази криптограма е създадена с по-голямо множество допустими стойности от настоящето.");
                }
